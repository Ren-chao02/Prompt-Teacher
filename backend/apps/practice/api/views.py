    ...
    def statistics(self, request):
        ...
        stats = {
            'overview': {
                ...
                'total_duration': base_qs.aggregate(total=Sum('duration_seconds'))['total'] or 0,
            },
...
class LLMConfigViewSet(viewsets.ModelViewSet):
    ...
    def test_saved(self, request, pk=None):
        """按已保存配置的 ID 测试连接（用数据库中的 api_key，无需前端回传）。

        解决前端编辑态无法测试已保存密钥的问题：list 接口不返回 api_key，
        编辑时输入框为空，无法用 test_connection 测试；此接口直接读取已保存配置。
        """
        config = self.get_object()
        from ..services.llm_service import llm_service
        result = llm_service.test_connection(
