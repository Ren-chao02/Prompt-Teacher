...
def api_topics(request):
    """按场景返回主题列表（供历史页筛选下拉框 AJAX 加载）。

    模板 practice_history.html 中场景切换时通过
    fetch("{% url "practice:api_topics" %}?scenario=<scenario_id>") 调用本接口，
    期望返回 {topics: [{id, topic_number, title}, ...]}。
    """
    scenario_id = request.GET.get('scenario', '').strip()
    if not scenario_id:
        return JsonResponse({'topics': []})

    topics = PracticeTopic.objects.filter(
        scenario__scenario_id=scenario_id,
        is_active=True
    ).order_by('order', 'topic_number').values('id', 'topic_number', 'title')
